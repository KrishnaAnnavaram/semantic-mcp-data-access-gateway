...
and a credential in `db/migrations/` is a credential in git forever. So V009
creates the role without a password and this script sets it from the
environment, connecting as the owner.

    MCP_READER_PASSWORD=... python -m src.mcp_data.bootstrap
...
from _db import connect as owner_connect, describe_target, load_dotenv  # noqa: E402
        ...
        with conn.cursor() as cur:
            ...
            if cur.fetchone() is None:
                LOGGER.error(
                    ...
                    "python .claude/loading/migrate.py", user)
                return 1

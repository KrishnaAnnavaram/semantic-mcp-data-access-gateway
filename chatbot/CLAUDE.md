...
Streamlit chatbot front-end for the smart MCP project. See the repo root `CLAUDE.md` for how this
fits into the overall system; see `README.md` for human-oriented setup instructions.

## What this service does

Takes a user's question, forwards it to the smart agent (owned by another teammate) over the REST
...
- `agent_client.py` — talks to the smart agent. `AgentClient` is a `Protocol` with two
...
in `.env` to point at a real smart agent.

"""CLI entry point for the MCP host.
    ...
    python -m mcp_servers.host --demo        run the end-to-end demo
    python -m mcp_servers.host --tools       list what the two servers offer
    python -m mcp_servers.host --isolation   prove the risk engine has no database access
    python -m mcp_servers.host --primitives  exercise all six MCP primitives
    python -m mcp_servers.host --ask "..."   answer a question with the smart agent
"""
...
def main() -> int:
    ...
    group.add_argument("--primitives", action="store_true",
                       help="exercise all six MCP primitives")
    group.add_argument("--ask", metavar="QUESTION",
                       help="answer a question with the agent driving both servers")
    parser.add_argument("--log-level", default="WARNING",
                        choices=["DEBUG", "INFO", "WARNING", "ERROR"])
    parser.add_argument("--interactive", action="store_true",
                        help="answer elicitations on the terminal instead of declining")
    args = parser.parse_args()

    # A model writing a minus sign as U+2212, or a curve note containing an
    # en dash, is enough to kill the whole run on a cp1252 console. The output
    # is diagnostic, so degrade the character rather than the command.
    ...
    if args.primitives:
        from .primitives import run_primitives  # noqa: PLC0415
        return asyncio.run(run_primitives())
    if args.ask:
        from .agent import run_ask  # noqa: PLC0415
        return asyncio.run(run_ask(args.ask, interactive=args.interactive))

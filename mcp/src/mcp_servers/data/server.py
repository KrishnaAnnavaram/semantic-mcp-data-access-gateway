...
Run: `python -m mcp_servers.data.server` (stdio). Diagnostics go to stderr - stdout
...
from mcp_servers.paths import DOCS_DIR as REPO_DOCS

from mcp.server import CacheHint, MCPServer
from mcp.server.mcpserver import ElicitationResult, Resolve
...
@server.tool(annotations=READ_ONLY, description=(
    ...
    "not a model. If the query spans both nominal and real series the caller is "
    "ASKED which one is meant (elicitation) rather than being handed both - a "
    "nominal par yield and a real yield must never share a curve. Answering "
    "filters the matches; declining returns them unfiltered and says so."
))
def search_series(
    query: str,
    data_key: str | None = None,
    limit: int = 10,
    # Filled by the resolver before this body runs, not by the model. When the
    # query spans two rate kinds this becomes an elicitation the client answers
    # by retrying the call; otherwise it costs nothing.
    choice: Annotated[
        ElicitationResult[interactive.RateKindChoice],
        Resolve(interactive.resolve_rate_kind),
    ...
    with connect() as conn:
        ...
        action = getattr(choice, "action", "accept")
        chosen = getattr(getattr(choice, "data", None), "rate_kind", None)

        # An answer only means something when there was a question. On the
        # unambiguous path the resolver returns the single matching kind
        # without asking, and reporting that as "elicited" would overstate
        # what the caller was actually consulted about.
        if not ambiguous:
            resolution, resolved = "unambiguous", None
        elif action == "accept" and chosen:
            resolution, resolved = "elicited", chosen
            rows = [r for r in rows if r["rate_kind"] == chosen]
        else:
            # Declined or cancelled: return everything and label it. Guessing a
            # side here is precisely the error elicitation exists to prevent.
            resolution, resolved = ("declined" if action == "decline" else "cancelled"), None

        return SeriesSearchResult(
            envelope=_envelope(conn), query=query,
            matches=[_series_info(r) for r in rows],
            ambiguous=ambiguous,
            resolved_rate_kind=resolved,  # type: ignore[arg-type]
...
#
# Two tools that need something only the client has: a filesystem boundary, and
# a language model. Both ask for it mid-call and both report what they got.
...
@server.tool(annotations=ToolAnnotations(
    ...
    "Write one day's curve to a CSV inside a directory the CLIENT has declared "
    "as a root. The server does not choose the destination and cannot write "
    "outside the roots it is given; if the client declares none, nothing is "
    "written and the refusal says so. filename must be a bare name - a path "
    "separator or '..' is refused rather than sanitised."
))
def export_curve_csv(
    ...
    observation_date: dt.date | None = None,
    roots: Annotated[
        ListRootsResult, Resolve(interactive.resolve_export_roots),
    ] = None,  # type: ignore[assignment]
) -> CurveExportResult:
    with connect() as conn:
        resolved = repo.resolve_curve_date(conn, curve_family, observation_date, "previous")
        if resolved is None:
            raise errors.date_no_data(
                (observation_date or interactive.today()).isoformat(), curve_family, [])
        rows = repo.get_curve(conn, curve_family, resolved)
        envelope = _envelope(conn, as_of=resolved)
        provenance = _provenance(rows)

    target, offered = interactive.contained_target(roots, filename)
    if target is None:
        reason = ("the client declared no usable filesystem root"
                  if not offered else
                  f"{filename!r} is not a bare filename, or would resolve outside "
                  f"the root it was joined to")
        return CurveExportResult(
            envelope=envelope, curve_family=curve_family, observation_date=resolved,
            roots_offered=offered, refused_reason=reason, provenance=provenance,
        )

    # quote_basis is a column, not a footnote: a CSV that drops it is exactly
    # the artefact that lets a discount rate end up on a par curve later.
    lines = ["series_code,display_name,rate_kind,quote_basis,tenor_months,"
             "observation_date,rate_percent"]
    for r in rows:
        lines.append(",".join(str(x) if x is not None else "" for x in (
            r["series_code"], (r["display_name"] or "").replace(",", " "),
            r["rate_kind"], r["quote_basis"], r.get("tenor_months"),
            r["observation_date"], r["rate_percent"],
        )))
    payload = "\n".join(lines) + "\n"
    target.write_text(payload, encoding="utf-8")
    LOGGER.info("wrote %d curve rows to %s", len(rows), target)

    return CurveExportResult(
        envelope=envelope, curve_family=curve_family, observation_date=resolved,
        written_path=str(target), bytes_written=len(payload.encode("utf-8")),
        row_count=len(rows), roots_offered=offered, provenance=provenance,
    )


@server.tool(annotations=READ_ONLY, description=(
    "Turn a dataset's terse caveat into desk-ready guidance. This server has no "
    "language model and no API key; it borrows the CLIENT's model via sampling "
    "for the wording only. The verbatim caveat is returned alongside the draft - "
    "where the two disagree, the verbatim text wins."
))
def brief_dataset_caveat(
    data_key: str,
    drafted: Annotated[
        CreateMessageResult, Resolve(interactive.resolve_caveat_briefing),
    ] = None,  # type: ignore[assignment]
) -> CaveatBriefing:
    source = interactive.caveat_source(data_key)
    if source is None:
        with connect() as conn:
            known = [{"data_key": d["data_key"], "title": d["title"]}
                     for d in repo.list_datasets(conn)]
        raise errors.unknown_entity("dataset", data_key, known)

    text = getattr(getattr(drafted, "content", None), "text", "") or ""
    if not text.strip():
        # The client answered but produced nothing usable. The caveat is the
        # load-bearing part, so the tool still succeeds - it just says the prose
        # is missing rather than inventing a substitute.
        text = ("[no briefing returned by the client's model — read the verbatim "
                "caveat below]")

    with connect() as conn:
        envelope = _envelope(conn, [data_key])
    return CaveatBriefing(
        envelope=envelope,
        data_key=str(source["data_key"]), title=str(source["title"]),
        briefing=text.strip(), verbatim_caveat=str(source["caveat"]),
        drafted_by_model=getattr(drafted, "model", None),
        first_observation=source["first_observation"],  # type: ignore[arg-type]
        last_observation=source["last_observation"],  # type: ignore[arg-type]
    )

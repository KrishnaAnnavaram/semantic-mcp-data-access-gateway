...
# The pipeline package already owns .env parsing; reuse it rather than keeping
# a second reader that can drift. `tools/verify_load.py` uses the same pattern.
...
def connection_settings() -> dict[str, Any]:
    ...
    if not password:
        raise ConfigurationError(
            ...
            "`python -m mcp_servers.data.bootstrap` once to set the password."
        )
    return {
